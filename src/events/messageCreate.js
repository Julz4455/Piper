  ...
  try {
    ...
    if(stamps.has(msg.author.id)) {
      ...
      if(now < exp) {
        ...
        return msg.channel.createMessage(`<@!${msg.author.id}>, You may not use the \`${command.name}\` command for another ${Math.trunc(left)} seconds`)
      }
    }
    ...
    try {
      ...
      console.log(`Some error occured while launching command ${command.name}: `)
      console.log(e)
    ...
    console.log(`Failure on messageCreate:`)
    console.log(e)
  }
}
